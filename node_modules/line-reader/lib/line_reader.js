(function() {
  ...
  function LineReader(fd, cb,option) {
    var filePosition   = option.position || 0,
    ...
    function nextLine(cb) {
      function getLine() {
        ...
        separatorIndex = -1;
        realPostion = realPostion + ret.length
        cb(ret,realPostion);
      }
  ...
  function open(filename, cb, option) {
    fs.open(filename, 'r', parseInt('666', 8), function(err, fd) {
      ...
      reader = new LineReader(fd, function(realPostion) {
        cb(null,reader);
      }, option);
    });
  }

  function eachLine(filename, cb, option) {
    var finalFn,
        asyncCb = cb.length == 4;
    ...
    open(filename, function(err,reader) {
      function newRead() {
      ...
      function readNext() {
        reader.nextLine(function(line,filePosition) {
          var last = !reader.hasNextLine();

          if (asyncCb) {
            cb(line, last, filePosition,continueCb);
          } else {
            if (cb(line, last, filePosition) !== false) {
              newRead();
            } else {
              finish();
              reader.close();
